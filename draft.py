...
def create_graph(n, m, s_node_degrees):
    ...
    s_degrees = sorted(s_node_degrees)
    c_degrees = [0] * m  # Initialize check node degrees
    for i, s_degree in enumerate(s_degrees):
        for _ in range(s_degree):
            # If it's the first edge for the current symbol node
            if c_degrees[0] == 0:
                c_index = 0
            else:
                # Find the lowest check node degree
                c_index = c_degrees.index(min(c_degrees))
            
            # Add the edge
            c_node = 'c{}'.format(c_index)
            G.add_edge(s_nodes[i], c_node)
            c_degrees[c_index] += 1
